...
in the normal way via the `children` list. The content for the drawer has to be added by calling `setDrawerContent(Node)`.
...
- Auto hiding: the drawer will close when the user clicks into the background (onto the glass pane)
