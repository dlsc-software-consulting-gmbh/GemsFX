"""Generated cartoon illustrations for the PhotoView manual."""
...
OUT = Path(__file__).resolve().parent.parent / "graphics" / "photo-view"
photo = "photo"
LINE = "#B7C2D0"
...
def cover():
    s=Sketch(1000,480,seed=440)
    s.window(60,35,880,400,title="PhotoView")
    if photo=="avatar":
...
def anatomy():
    s=Sketch(1120,540,seed=440+1); s.window(50,42,610,420,title="PhotoView")
    if photo=="avatar":
...
def flow():
    ...
    if photo=="avatar": _avatar(s,430,260,45,"LD",TEAL_PALE); s.text(500,350,"image → initials → icon",size=22,fill=INK_SOFT)
    elif photo=="photo": _photo(s,390,235,190,150,"circle"); s.text(500,350,"photoZoom and translate feed crop",size=22,fill=INK_SOFT)
    elif photo=="svg": _svg_icon(s,385,230,210,150); s.text(500,350,"jSVG renders BufferedImage → FX Image",size=22,fill=INK_SOFT)
    elif photo=="before": _before(s,365,225,260,150,.55); s.text(500,350,"dividerPosition controls both clips",size=22,fill=INK_SOFT)
    elif photo=="masked": _masked(s,345,245,310,105); s.text(500,370,"translateX controls edge gradients",size=22,fill=INK_SOFT)
    else: _screen(s,370,235,260,145,"scaled union",TEAL_PALE); s.text(500,365,"real geometry scaled to fit",size=22,fill=INK_SOFT)
    return s

def layout():
    s=Sketch(1000,430,seed=440+4)
    if photo=="avatar":
...
def styling():
    s=Sketch(1000,390,seed=440+5)
    s.box(80,65,360,220,fill=PAPER,stroke=INK,radius=14,shadow=True)
    if photo=="avatar": _avatar(s,260,175,58,"ST",TEAL_PALE)
    elif photo=="photo": _photo(s,155,95,190,160,"circle")
    elif photo=="svg": _svg_icon(s,145,95,200,160)
    elif photo=="before": _before(s,120,100,260,150,.5)
    elif photo=="masked": _masked(s,110,120,300,120)
