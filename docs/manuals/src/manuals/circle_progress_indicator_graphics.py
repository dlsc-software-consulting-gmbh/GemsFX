"""Generates cartoon illustrations for the CircleProgressIndicator manual."""
...
OUT = Path(__file__).resolve().parent.parent / "graphics" / "circle-progress-indicator"
IS_CIRCLE = True
IS_SEMI = False
TITLE = "CircleProgressIndicator"
...
def layout():
    s = Sketch(1050, 500, seed=115)
    cx, cy = 420, 230 if not IS_SEMI else 260
    radius = 145 if not IS_SEMI else 130
