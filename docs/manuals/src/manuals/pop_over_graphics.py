...
def _popover(s,x,y,w,h,side="top"):
    ...
    if side=="bottom": s.line(x+w/2-18,y+h,x+w/2,y+h+25,stroke=INK,width=3); s.line(x+w/2,y+h+25,x+w/2+18,y+h,stroke=INK,width=3)
    if side=="left": s.line(x,y+h/2-18,x-25,y+h/2,stroke=INK,width=3); s.line(x-25,y+h/2,x,y+h/2+18,stroke=INK,width=3)
    if side=="right": s.line(x+w,y+h/2-18,x+w+25,y+h/2,stroke=INK,width=3); s.line(x+w+25,y+h/2,x+w,y+h/2+18,stroke=INK,width=3)
...
def cover():
 s=Sketch(1000,480,seed=311); s.window(70,45,860,360,title="Owner window"); _button(s,435,330,120,'Owner',INDIGO_PALE); _popover(s,350,110,300,170,'bottom'); return s
def anatomy():
 s=Sketch(1120,560,seed=312); _button(s,260,410,110,'Owner',INDIGO_PALE); _popover(s,170,170,300,180,'bottom'); keys=[('1',190,180,'background path',95),('2',220,215,'content BorderPane',165),('3',470,170,'border path',235),('4',255,178,'root StackPane copies style classes',305),('5',320,370,'arrow path points at owner',380),('6',345,395,'offset overlap',460)]
...
def locations():
 s=Sketch(1000,430,seed=314); _button(s,450,185,100,'Owner',INDIGO_PALE); _popover(s,370,40,260,105,'bottom'); _popover(s,370,285,260,105,'top'); _popover(s,665,130,210,140,'left'); _popover(s,125,130,210,140,'right'); s.label(500,405,'Preferred arrowLocation may flip to stay on screen; computedArrowLocation reports the effective side',size=18,fill=INDIGO,bg=INDIGO_PALE); return s
def detach():
 s=Sketch(1000,430,seed=315); _button(s,180,300,100,'Owner',INDIGO_PALE); _popover(s,120,120,220,120,'bottom'); s.arrow(350,180,570,160,bend=.1,stroke=AMBER,width=2.5,head=12); s.box(590,95,290,170,fill=PAPER,stroke=INK,radius=14,shadow=True); s.label(735,125,'detached',size=18,fill=ROSE,bg=ROSE_PALE); _txt(s,735,185,'No arrow + autoHide false',19,ROSE,bold=True); return s
