...
OUT = Path(__file__).resolve().parent.parent / "graphics" / "paging-list-view"
LINE="#B7C2D0"
...
def cover():
 s=Sketch(1000,480,seed=211); s.window(60,35,880,400,title="PagingListView"); _list(s,150,85,700,245,5); _pager(s,150,350,700); return s
def anatomy():
 s=Sketch(1120,560,seed=212); _list(s,70,80,450,280,5); _pager(s,70,385,450); keys=[("1",140,108,"inner ListView",95),("2",300,250,"itemsOnCurrentPage",165),("3",320,410,"PagingControls",245),("4",520,90,"StackPane overlays placeholder",325),("5",500,405,"optional ScrollPane wrapper",405),("6",385,470,"loader drives totalItemCount",485)]
 ...
 return s
def loading():
 s=Sketch(1000,430,seed=213); boxes=[("page/pageSize change","reload()"),("LoadingService","delay + background Task"),("loader callback","PagingLoadResponse"),("itemsOnCurrentPage","ListView items")]
 for i,(a,b) in enumerate(boxes): x=70+i*225; s.box(x,130,175,105,fill=INDIGO_PALE if i%2==0 else TEAL_PALE,stroke=INDIGO if i%2==0 else TEAL,radius=12,shadow=True); s.text(x+87,165,a,size=18,fill=INK,bold=True); s.text(x+87,198,b,size=16,fill=INK_SOFT)
 for x in [245,470,695]: s.arrow(x,182,x+48,182,bend=0,stroke=AMBER,width=2.5,head=12)
 return s
def states():
 s=Sketch(1000,430,seed=214); _list(s,60,75,260,220,5); s.label(190,330,"OK",size=19,fill=TEAL,bg=TEAL_PALE); _list(s,370,75,260,220,2); s.blob(500,185,38,38,fill=AMBER_PALE,stroke=AMBER); s.text(500,192,"…",size=30,fill=AMBER); s.label(500,330,"LOADING after commit delay",size=18,fill=AMBER,bg=AMBER_PALE); s.box(690,75,260,220,fill=PAPER,stroke=INK,radius=12,shadow=True); s.text(820,190,"No items",size=22,fill=INK_SOFT); s.label(820,330,"placeholder",size=19,fill=INK_SOFT,bg=SLATE_PALE); return s
def layout():
 s=Sketch(1000,430,seed=215); _pager(s,90,70,360); _list(s,90,155,360,190,4); s.label(270,380,"controls TOP",size=18,fill=INDIGO,bg=INDIGO_PALE); _list(s,560,70,360,190,4); _pager(s,560,285,360); s.label(740,380,"controls BOTTOM",size=18,fill=INDIGO,bg=INDIGO_PALE); return s
DRAWINGS={"cover.svg":cover,"anatomy.svg":anatomy,"loading.svg":loading,"states.svg":states,"layout.svg":layout}
