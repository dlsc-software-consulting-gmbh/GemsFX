"""Generated cartoon illustrations for the BeforeAfterView manual."""
...
OUT = Path(__file__).resolve().parent.parent / "graphics" / "before-after-view"
before = "before"
LINE = "#B7C2D0"
...
def cover():
    s=Sketch(1000,480,seed=480)
    s.window(60,35,880,400,title="BeforeAfterView")
    if before=="avatar":
...
def anatomy():
    s=Sketch(1120,540,seed=480+1); s.window(50,42,610,420,title="BeforeAfterView")
    if before=="avatar":
...
def flow():
    ...
    if before=="avatar": _avatar(s,430,260,45,"LD",TEAL_PALE); s.text(500,350,"image → initials → icon",size=22,fill=INK_SOFT)
    elif before=="photo": _photo(s,390,235,190,150,"circle"); s.text(500,350,"photoZoom and translate feed crop",size=22,fill=INK_SOFT)
    elif before=="svg": _svg_icon(s,385,230,210,150); s.text(500,350,"jSVG renders BufferedImage → FX Image",size=22,fill=INK_SOFT)
    elif before=="before": _before(s,365,225,260,150,.55); s.text(500,350,"dividerPosition controls both clips",size=22,fill=INK_SOFT)
    elif before=="masked": _masked(s,345,245,310,105); s.text(500,370,"translateX controls edge gradients",size=22,fill=INK_SOFT)
    else: _screen(s,370,235,260,145,"scaled union",TEAL_PALE); s.text(500,365,"real geometry scaled to fit",size=22,fill=INK_SOFT)
    return s

def layout():
    s=Sketch(1000,430,seed=480+4)
    if before=="avatar":
...
def styling():
    s=Sketch(1000,390,seed=480+5)
    s.box(80,65,360,220,fill=PAPER,stroke=INK,radius=14,shadow=True)
    if before=="avatar": _avatar(s,260,175,58,"ST",TEAL_PALE)
    elif before=="photo": _photo(s,155,95,190,160,"circle")
    elif before=="svg": _svg_icon(s,145,95,200,160)
    elif before=="before": _before(s,120,100,260,150,.5)
    elif before=="masked": _masked(s,110,120,300,120)
