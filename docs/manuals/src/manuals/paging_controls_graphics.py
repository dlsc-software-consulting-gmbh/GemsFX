...
OUT = Path(__file__).resolve().parent.parent / "graphics" / "paging-controls"
LINE="#B7C2D0"
...
def cover():
 s=Sketch(1000,480,seed=201); s.window(70,50,860,350,title="Paged results"); _pager(s,150,195,700); s.cursor(590,245,1.2); s.sparkle(578,238,13,AMBER); return s
def anatomy():
 s=Sketch(1120,560,seed=202); _pager(s,70,130,680); keys=[("1",90,157,"messageLabel from provider",100),("2",315,184,"first / previous buttons",165),("3",500,184,"page-button grid",245),("4",681,184,"next / last buttons",325),("5",674,154,"page size ChoiceBox",405),("6",390,205,"needed property hides whole control",485)]
 for k,x,y,t,ly in keys: s.badge(800,ly,k); s.text(832,ly+6,t,size=19,anchor="start"); s.arrow(780,ly,x,y,bend=.04,stroke=MUTED,width=1.6,head=9)
 return s
def page_math():
 s=Sketch(1000,420,seed=203); s.box(100,80,800,90,fill=PAPER,stroke=INK,radius=12); s.text(500,132,"pageCount = totalItemCount / pageSize, rounded up",size=24,fill=INDIGO,bold=True); 
 for i,(a,b) in enumerate([("87 items","10 per page → 9 pages"),("10 items","10 per page → 1 page"),("0 items","0 pages + message")]): s.box(130+i*260,230,220,70,fill=SLATE_PALE,stroke=LINE,radius=10); s.text(240+i*260,258,a,size=19); s.text(240+i*260,284,b,size=17,fill=INK_SOFT)
 return s
def modes():
 s=Sketch(1000,420,seed=204); _pager(s,70,90,380); s.label(260,210,"SHOW_PAGE_BUTTONS",size=18,fill=INDIGO,bg=INDIGO_PALE); _pager(s,550,90,380); s.label(740,210,"SHOW_ARROW_BUTTONS / HIDE",size=18,fill=INDIGO,bg=INDIGO_PALE); return s
def keyboard():
 s=Sketch(1000,420,seed=205); _pager(s,150,120,700); keys=[("RIGHT / PAGE_DOWN","nextPage()"),("LEFT / PAGE_UP","previousPage()"),("HOME","firstPage()"),("END","lastPage()")]
 for i,(a,b) in enumerate(keys): s.box(150+i*205,260,170,70,fill=AMBER_PALE,stroke=AMBER,radius=10); s.text(235+i*205,288,a,size=15); s.text(235+i*205,314,b,size=17,fill=AMBER,bold=True)
 return s
DRAWINGS={"cover.svg":cover,"anatomy.svg":anatomy,"page-math.svg":page_math,"modes.svg":modes,"keyboard.svg":keyboard}
